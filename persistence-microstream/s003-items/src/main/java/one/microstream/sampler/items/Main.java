...
public class Main {
  ...
  public static final String DATA_PATH = "data-s003";
  ...
  public Main() {
    this.storageManager = EmbeddedStorage.start(new DataRoot(), new File(DATA_PATH));
  }
  ...
  private void command() {
    ...
    if (command.startsWith("a ")) {
      final String title = command.substring(2)
                                  .trim();
      if (title.isEmpty()) {
        usage();
      } else {
        final List<Item> items = this.root()
                                     .getItems();
      ...
      try {
        final int index = Integer.parseInt(command.substring(2)
                                                  .trim());
        final List<Item> items = this.root()
                                     .getItems();
      ...
      final String term = command.substring(2)
                                 .trim()
                                 .toLowerCase();
      if (term.isEmpty()) {
        usage();
